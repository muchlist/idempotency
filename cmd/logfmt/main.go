...
func main() {
	...
	for scanner.Scan() {
		...
		sortableListKV := make([]kv, 0, len(m))
		...
		for k, v := range m {
			...
			sortableListKV = append(sortableListKV, kv{
				key:   k,
				value: v,
			})
		}

		sort.Slice(sortableListKV, func(i, j int) bool {
			return sortableListKV[i].key > sortableListKV[j].key
		})

		for _, v := range sortableListKV {

			// It's nice to see the key[value] in this format
			// especially since map ordering is random.
			b.WriteString(fmt.Sprintf("%s[%v]: ", v.key, v.value))
		}
